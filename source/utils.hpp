...
void drawTable(std::unique_ptr<tsl::elm::List>& list, std::vector<std::string>& sectionLines, std::vector<std::string>& infoLines,
    ...
    auto getTextColor = [](const std::string& colorStr, auto defaultColor) {
        ...
        if (colorStr == "accent") return tsl::accentTextColor;
        if (colorStr == "sectioninfo") return tsl::sectitleTextColor;
...
void addHelpInfo(std::unique_ptr<tsl::elm::List>& list) {
    ...
    drawTable(list, sectionLines, infoLines, xOffset, 20, 12, 3, SECTITLE_STR, ACCENT_STR, DEFAULT_STR, LEFT_STR, true);
...
void addCreditInfo(std::unique_ptr<tsl::elm::List>& list, auto& packageHeader, std::string type = OVERLAY_STR) {
    ...
    // Drawing the table with section lines and info lines
    drawTable(list, sectionLines, infoLines, xOffset, 20, 12, 5, SECTITLE_STR, ACCENT_STR, DEFAULT_STR, LEFT_STR, true);
}
